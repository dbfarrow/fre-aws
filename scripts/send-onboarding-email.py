...
def build_body_admin(username, project, sso_start_url, aws_region,
    ...
    if account_id:
        aws_config_content = (
            f"# Admin profile — use for admin.sh ({project}-admin-access)\n"
            f"[profile claude-code]\n"
            f"sso_session = {project}-admin\n"
            f"sso_account_id = {account_id}\n"
            f"sso_role_name = {project}-admin-access\n"
            f"region = {aws_region}\n"
            f"\n"
            f"# Developer profile — use for user.sh connect ({project}-developer-access)\n"
            f"[profile claude-code-dev]\n"
            f"sso_session = {project}-dev\n"
            f"sso_account_id = {account_id}\n"
            f"sso_role_name = {project}-developer-access\n"
            f"region = {aws_region}\n"
            f"\n"
...
def build_html_admin(username, project, sso_start_url, aws_region,
    ...
    if account_id:
        aws_config_content = (
            "# Admin profile — use for admin.sh (" + project + "-admin-access)\n"
            "[profile claude-code]\n"
            "sso_session = " + project + "-admin\n"
            "sso_account_id = " + account_id + "\n"
            "sso_role_name = " + project + "-admin-access\n"
            "region = " + aws_region + "\n"
            "\n"
            "# Developer profile — use for user.sh connect (" + project + "-developer-access)\n"
            "[profile claude-code-dev]\n"
            "sso_session = " + project + "-dev\n"
            "sso_account_id = " + account_id + "\n"
            "sso_role_name = " + project + "-developer-access\n"
            "region = " + aws_region + "\n"
            "\n"
